...
func (*VideoDao) GetVideoSumByTime(latest_time time.Time) (int, error) {
	var videos []Video
	result := db.Where("create_time < ?", latest_time).Find(&videos)
	if result.Error != nil {
...
// // 先排序再比较(nlogn)
//
//	func (*VideoDao) GetVideoSumByTimeB(latest_time time.Time)(int,error){
//		var videos []Video
//		result := db.Order("create_time DESC").Where("create_time < ?",latest_time).Find(&videos)
//			if result.Error!=nil{
//			return 0,errors.New("先排序再比较出错")
//		}
//		return int(result.RowsAffected),nil
//	}
//
...
func (*VideoDao) GetVideoByTime(latest_time time.Time) (*Video, error) {
	var v Video
	err := db.Order("create_time DESC").Where("create_time < ?", latest_time).First(&v).Error
	if err != nil {
		return nil, err
	}
	return &v, nil
...
// 根据用户id返回视频数量
func (*VideoDao) GetVideoCountByUserId(vid int64) (int32, error) {
	result := db.Find(&Video{Id: vid})
	if result.Error != nil {
...
// 根据用户id返回指定数量的视频
func (*VideoDao) GetVideoByUserId(uid int64, count int32) (*[]Video, error) {
	videos := make([]Video, count)
	result := db.Order("create_time desc").Limit(int(count)).Find(&videos)
	if result.Error != nil {
...
func (*VideoDao) IsUserIdValid(uid int64) (bool, error) {
	result := db.Find(&Video{UserId: uid})
	if result.Error != nil {
		return false, errors.New("按照用户id查询视频是否存在时出错")
	}
	if result.RowsAffected == 0 {
		return false, nil
	}
